...
Context&
Context3D::Begin(RenderTarget& target, u32 frameIndex) {
...
Context&
Context3D::UsingPipeline(Pipeline& pipeline) {
	auto contextData = SPtrFromHandle<ContextData>(impl);
...
Context&
Context3D::PaintWith(Texture& texture) {
	auto contextData = SPtrFromHandle<ContextData>(impl);
...
Context&
Context3D::DrawTriangleList(VertexBuffer& vbuffer) {
	auto contextData = SPtrFromHandle<ContextData>(impl);
...
Context&
Context3D::Finish(RenderTarget& target, u32 frameIndex) {
	auto contextData = SPtrFromHandle<ContextData>(impl);
	auto targetData = SPtrFromHandle<RenderTargetData>(target.Get());
